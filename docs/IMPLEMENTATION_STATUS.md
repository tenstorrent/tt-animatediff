...
**Status: ✅ Complete (v0.7.0, bugs fixed v0.8.0)**

Full `guoyww/animatediff-motion-adapter-v1-5-2` temporal attention injected at 7 points
(down0/1/2, mid, up0/1/2) in the SD 1.4 TTNN UNet via CPU round-trip. Enable with
`--motion-adapter`. Skip high-resolution up-blocks for ~6× speedup:
`--motion-adapter-skip up1 up2`.

**Two bugs fixed in v0.8.0:**
1. `nn.Linear` weight transpose — `load_weights()` now applies `.T.contiguous()`.
2. Partial kernel replaced with full `AnimateDiffTransformer3D.forward()` — energy
   ratios dropped from >2.0 to <1.25, producing real images instead of noise.

---

## World's Fair Showcase

**Status: ✅ Complete (v0.7.0, regenerated v0.8.0)**

9 World's Fair prompts × 3 quality tiers (Q1: PNDM/25-step/16fr/4-chip, Q2: Lightning/8-step/8fr/4-chip,
Q3: PNDM/25-step/8fr/1-chip) + Unisphere 100-year chain at `docs/worlds-fair.html`.
Generated by `scripts/generate_worlds_fair.py`.

---

## TT-Lang Temporal Attention (Simulator Track)

**Status: ✅ Complete (v0.7.0)**

`animatediff_ttnn/ttlang/` — 3 DSL kernels (QKV, SDPA, output projection) in TT-Lang
functional simulator. 9 tests, all PCC > 0.999. Hardware smoke test on dual P300c:
PCC > 0.99 at C=320/640/1280.
